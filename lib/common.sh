...
_dev_server_validate_change() {
  ...
  case "$change_id" in
  tmux.config | shell.config | ai.instructions | desktop.session | ssh.config | docker.config | skid.unit | skid.runtime | skid.integration | codex.runtime | tailscale.serve | system.reboot) ;;
